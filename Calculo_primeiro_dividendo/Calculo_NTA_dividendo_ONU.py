...
# =============================================================================
# ETAPA 2.2: INTEGRAÇÃO DAS PROJEÇÕES DA ONU (2030 a 2100)
# =============================================================================
print(" -> Lendo projeções da ONU (2030 a 2100)...")

df_onu = pd.read_excel('Base_Onu_dividendo.xlsx')
anos_projecao = [2030, 2040, 2050, 2060, 2070, 2080, 2090, 2100]

# Filtra apenas as décadas exatas que queremos projetar
df_onu_filtrado = df_onu[df_onu['Year'].isin(anos_projecao)].copy()
df_onu_filtrado.set_index('Year', inplace=True)

for ano_proj in anos_projecao:
    linha_ano = df_onu_filtrado.loc[ano_proj]

    # Cria o vetor de população de 91 posições (0 a 90) zerado
    pop_vetor_onu = np.zeros(91)

    # Varre todas as colunas para agrupar as idades
    for col in df_onu_filtrado.columns:
        val = linha_ano[col]
        if pd.isna(val): continue

        # Limpa o texto da coluna para extrair a idade (ex: "100+" vira "100")
        col_str = str(col).lower().replace('+', '').strip()
        try:
            idade = int(col_str)
            # Agrupa tudo que for >= 90 no índice 90
            if idade >= 90:
                pop_vetor_onu[90] += val
            else:
                pop_vetor_onu[idade] += val
        except ValueError:
            pass  # Pula colunas que não representam idades

    # Aplica o perfil NTA sobre a pirâmide projetada da ONU
    for ano_base_nta in anos_nta:
        L_t = np.sum(pop_vetor_onu * perfis_nta[ano_base_nta]['YL'])
        N_t = np.sum(pop_vetor_onu * perfis_nta[ano_base_nta]['C'])
        SR_t = L_t / N_t
        resultados_sr.append(
            {'Ano_Base_NTA': ano_base_nta, 'Ano_Censo': ano_proj, 'L_t (Produtores)': L_t, 'N_t (Consumidores)': N_t,
             'Razao_Suporte_SR': SR_t})
...
nome_arquivo_saida = "resultado_dividendo_brasil_NTA_ate_2100.xlsx"
df_final.to_excel(nome_arquivo_saida, index=False)
print(f"Sucesso absoluto! Matriz completa salva como '{nome_arquivo_saida}'.")
